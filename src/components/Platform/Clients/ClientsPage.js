...
function ClientsPage() {
  ...
  const fetchClients = useCallback(async (page, search) => {
    ...
    setIsLoading(true);
    try {
      const data = await clientServices.getClients(token, search, page, ITEMS_PER_PAGE);
  ...
  return (
    <div style={styles.clientsPageContainer}>
      ...
      <div style={styles.tableControlsHeader}>
        <div style={styles.searchBox}><i className="fa-solid fa-magnifying-glass" style={styles.searchBoxIcon}></i><input type="text" placeholder="Buscar por nome, CPF ou telefone..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} style={styles.searchInput} /></div>
        <div style={styles.actionsGroup}>
            <button onClick={() => navigate('/clients/create')} style={styles.addClientButton}><i className="fa-solid fa-plus"></i> Adicionar Cliente</button>
        </div>
      </div>

      <div style={styles.clientsTableCard}>
        <table style={styles.clientsTable}>
          <thead><tr><th style={styles.tableCell}>Nome</th><th style={styles.tableCell}>CPF/CNPJ</th><th style={styles.tableCell}>Email</th><th style={styles.tableCell}>Celular</th><th style={styles.tableCell}>Saldo em Conta</th></tr></thead>
          <tbody>
            {isLoading ? (
              <tr><td colSpan="5" style={{textAlign: 'center', padding: '20px'}}>Carregando...</td></tr>
                ...
                <tr key={client.id} onClick={() => navigate(`/platform/clients/${client.id}`)} onMouseEnter={() => setHoveredRow(client.id)} onMouseLeave={() => setHoveredRow(null)} style={{...styles.tableRow, ...(hoveredRow === client.id && styles.tableRowHover)}}>
                    ...
                    <td style={styles.tableCell}>{formatCurrency(client.balance)}</td>
                </tr>
                ))
            ) : (
