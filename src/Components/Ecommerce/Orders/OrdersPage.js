...
const CustomDropdown = ({ options, selected, onSelect, placeholder }) => {
  ...
  return (
    <div className="custom-dropdown-container-ord" ref={wrapperRef}>
      ...
      {isOpen && (
        <ul className="dropdown-list-ord">
          {options.map((option) => (
            <li
              key={option.value}
              onClick={() => {
...
const OrderDetailsModal = ({ order, onUpdateStatus, onClose, isClosing }) => {
  ...
  const whatsappLink = `https://wa.me/55${order.client.phoneNumber.replace(
    /\D/g,
    ""
  )}?text=${encodeURIComponent(
    `Olá ${order.client.name}, sobre seu pedido #${order.id}...`
  )}`;
  ...
  return (
    <div
      className={`modal-backdrop-ord ${isClosing ? "closing" : ""}`}
      onClick={onClose}
    >
      <div
        className={`modal-content-ord ${isClosing ? "closing" : ""}`}
        onClick={(e) => e.stopPropagation()}
      >
        <div className="modal-header-ord">
        ...
        <div className="order-details-body">
          <div className="details-grid">
            <div className="detail-section client-info">
              <h4>
                <i className="fa-solid fa-user"></i> Cliente
              </h4>
              <p>
                <strong>ID:</strong> #{order.client.id}
              </p>
              <p>
              ...
              <p>
                <strong>Telefone:</strong> {order.client.phoneNumber}
              </p>
            </div>
            <div className="detail-section shipping-info">
              <h4>
                <i className="fa-solid fa-truck"></i> Endereço de Entrega
              </h4>
              <p>{fullAddress}</p>
            </div>
            {order.consultant && (
              <div className="detail-section consultant-info">
                <h4>
                  <i className="fa-solid fa-user-tie"></i> Consultor Associado
                </h4>
                <p>
                  <strong>ID:</strong> #{order.consultant.id}
                </p>
                <p>
                  <strong>Nome:</strong> {order.consultant.name}
                </p>
              </div>
            )}
          </div>
          ...
          <div className="detail-section payment-shipping">
            ...
            <div>
              <h4>
                <i className="fa-solid fa-info-circle"></i> Status do Pedido
              </h4>
              <select
                className="status-select-ord"
                value={currentStatus}
                onChange={(e) => setCurrentStatus(e.target.value)}
              >
                <option value="PendingPayment">Aguardando Pagamento</option>
              ...
              <button
                ...
                onClick={() => onUpdateStatus(order.id, currentStatus)}
              >
                Atualizar Status
              </button>
            </div>
    ...
    <div className="orders-page-container">
      ...
      <section className="order-kpi-card">
        <div className="kpi-total">
          ...
          <span>{allOrdersForKpi.length}</span>
        </div>
        <div className="kpi-divider"></div>
        <div className="kpi-status-breakdown">
          <div className="status-item">
            <span>{orderStatusCounts["PendingPayment"] || 0}</span>
            <p>Pag. Pendente</p>
          </div>
          <div className="status-item">
            <span>{orderStatusCounts["Processing"] || 0}</span>
            <p>Processando</p>
          </div>
          <div className="status-item">
            <span>{orderStatusCounts["Shipped"] || 0}</span>
            <p>Enviados</p>
          </div>
          <div className="status-item">
            <span>{orderStatusCounts["Delivered"] || 0}</span>
            <p>Entregues</p>
          </div>
          <div className="status-item">
            <span>{orderStatusCounts["Cancelled"] || 0}</span>
            <p>Cancelados</p>
          </div>
        </div>
      </section>
            ...
            {isLoading ? (
              ...
              orders.map((order) => (
                <tr key={order.id} onClick={() => handleOpenModal(order)}>
                  ...
                  <td>
                    <span
                      className={`status-badge-ord status-${order.status.toLowerCase()}`}
                    >
                      {order.status}
                    </span>
                  </td>
