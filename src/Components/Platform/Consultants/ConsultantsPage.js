...
function ConsultantsPage() {
    ...
    const fetchConsultants = useCallback(async (page, search) => {
        if (!token) return;
        setIsLoading(true);
        try {
            const data = await consultantService.getConsultants(token, search, page, 10);
    ...
    return (
        <div style={styles.pageContainer}>
            ...
            <div style={styles.controlsHeader}>
                ...
                <button style={styles.createButton} onClick={() => navigate('/platform/consultants/create')}>
                    ...
                    <tbody>
                        {isLoading ? (
                            ...
                            consultants.map(consultant => (
                                <tr key={consultant.id} style={{ ...styles.tableRow, ...(hoveredRow === consultant.id && styles.tableRowHover) }} onMouseEnter={() => setHoveredRow(consultant.id)} onMouseLeave={() => setHoveredRow(null)} onClick={() => handleNavigate(consultant.id)}>
                                    ...
                                    <td style={styles.tableCell}>{formatServices.formatCpfCnpj(consultant.cpfCnpj)}</td>
                                    <td style={styles.tableCell}>
                                        <span style={{ ...styles.statusBadge, ...styles[statusMap[consultant.status] === 'Ativo' ? 'statusAtivo' : 'statusInativo'] }}>
                            ...
                            <tr><td colSpan="5" style={{ textAlign: 'center', padding: '20px' }}>Nenhum consultor encontrado.</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
